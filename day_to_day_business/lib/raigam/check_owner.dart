...
class _CheckOwnerState extends State<CheckOwner> {
  ...
  Stream owners;
    ...
    var  qn = await firestore.collection('Raigam Cheque').where('Name' , isEqualTo: name).snapshots();
    return qn;
  }
  ...
  void initState() {
    ...
    _updateData(selectDoc , newValue){
      Firestore.instance.collection('Raigam Cheque').document(selectDoc).updateData(newValue);
    }
